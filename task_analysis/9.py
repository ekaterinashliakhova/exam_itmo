...
average_usd = fraud_transactions['amount_usd'].mean()

# Округляем вверх до целого
rounded_average = math.ceil(average_usd)

# Выводим результаты
print(f"Среднее значение до округления: {average_usd:.2f}")
print(f"Среднее значение после округления вверх: {rounded_average}")